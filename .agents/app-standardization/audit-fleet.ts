...
function getSecret(project: string, key: string): string {
    try {
        return execSync(
            `doppler secrets get "${key}" --project "${project}" --config prd --plain`,
...
function checkHubSpoke(project: string): { ok: boolean; issues: string[] } {
    ...
    try {
        const raw = execSync(
            `doppler secrets --project "${project}" --config prd --json`,
            { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }
        )
...
function main() {
    ...
    for (const app of apps.sort()) {
        ...
        console.log(` ${name} ${structIcon} struct  ${layerIcon} layer  ${ga}  ${gsc}  ${inow}  ${ph}  ${hub}`)
    }
