---
description: Maximize efficiency in getting zero errors and zero warnings across the fleet
---
# Zero Warnings & Errors Workflow

This workflow is designed to systematically burn down all ESLint/TypeScript/Vue warnings and errors across the Narduk Nuxt Template fleet.

The "Zero Errors & Warnings Policy" is a strict institutional standard. **We do not suppress warnings**; we root-cause and fix them.
...
2. **Fleet Blitz (Optional)**: If there are many fixable warnings, run `pnpm quality:fleet --fix` to automatically resolve what can be automated across the whole fleet.
...
2. Run targeted quality to see details: `pnpm quality:fleet --filter=APP_NAME --no-pull`.
...
3. **ESLint Warnings**: Formatting, logic patterns (e.g., `unicorn/prefer-at`, `unicorn/prefer-string-replace-all`).
